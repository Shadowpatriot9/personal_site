...
function Muse() {
  const { project } = useProjectMetadata('muse');
  const projectTitle = project?.title || 'Muse';
  const projectDescription = project?.description || 'Automated Audio Equalizer';

  useEffect(() => {
    logger.pageView(`${projectTitle} Project Page`, {
      project: project?.id || 'muse',
